...
export default class Store {
  ...
  define(name, url, schema={}) {
    if (this.models.hasOwnProperty(name))
      throw new Error(`Model "${name}" already exists.`);
    this.models[name] = new Model(name, url, schema, {store: this});
    this._cache[name] = new Set();
    this[capitalize(name)] = {
      create: (state, cache) => this.createRecord(name, state, cache),
      get: (id, sync) => this.get(name, id, sync),
      all: (sync) => this.get(name, sync),
      one: () => this.one(name),
      alwaysOne: (sync) => this.alwaysOne(name)
    };
  }
