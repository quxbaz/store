...
export default class Store {
  ...
  getModelDeps(modelName) {
    let buildDeps = (model, deps) => {
      deps.add(model.name);
      Object.keys(model.schema).map(
        key => model.schema[key].modelName
      ).filter(
        modelName => !deps.has(modelName)
      ).forEach((modelName) => {
        buildDeps(this.models[modelName], deps);
      });
      return deps;
    };
    return Array.from(buildDeps(this.models[modelName], new Set()));
  }
  ...
  get(modelName, id) {
    ...
    // <TODO>
    // let deps = this.getModelDeps(modelName);
